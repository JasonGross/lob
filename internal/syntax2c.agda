...
_≟t_ : ∀ {Γ T} → SemiDecidable {A = Tm Γ T} _≡_
_≟T_ : ∀ {Γ} → SemiDecidable {A = Ty Γ} _≡_
_≟v_ : ∀ {Γ T} → SemiDecidable {A = Var Γ T} _≡_
...
vₙ : ∀ {Γ} (Δ : Γ ++) (B : Ty Γ) {A} → Var (++ Δ) A → Var (++ (_cw_ {Γ} B Δ)) (Wₙ Δ B A)
...
data Tm (Γ : Con) where
--  _‘’ₐ_ : ∀ {A B} → Tm Γ (‘Π’ A B) → (a : Tm Γ A) → Tm Γ (B ‘’ a)
...
cast-Var : ∀ {Γ} (T T' : Ty Γ) → Var Γ T → Var Γ T'
cast-Var {Γ} T T' v = helper T' (T ≟T T')
  where helper : ∀ T' → SemiDec (T ≡ T') → Var Γ T'
        helper ._ (just refl) = v
        helper _ nothing = v⊥

vₙ (ε Γ) A v = vs v
vₙ (Δ , X) A vz = cast-Var _ _ vz -- ACK!
vₙ (Δ , X) A (vs {A = B} v) = cast-Var _ _ (vs (vₙ Δ A v))
vₙ (Δ , X) A v⊥ = v⊥
...
vₙ-good : ∀ {Γ} (Δ : Γ ++) (B : Ty Γ) {A} → (v : Var (++ Δ) A)
  → (vₙ {Γ} Δ B {A} v ≡ v⊥)
  → v ≡ v⊥
vₙ-good (ε ._) B vz ()
vₙ-good (ε ._) B₁ (vs v) ()
vₙ-good (ε Γ) B v⊥ p = refl
vₙ-good (Δ , x) B v⊥ p = refl
vₙ-good (Δ , U) B vz ()
vₙ-good (Δ , ‘Π’ x x₁) B vz p = {!!}
vₙ-good (Δ , x) B (vs {._} {A} {._} v) p = {!!}
