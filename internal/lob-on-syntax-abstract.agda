...
module _
  ...
  module _
    ...
    (𝟙 : ∀ {Γ} → TySyntax Γ)
--    (𝟙-law : ∀ {Γ}
    (□ : ∀ {Γ} → TySyntax Γ → TySyntax Γ) -- quotes the context, too
    (□𝒰 : ∀ {Γ} → TySyntax Γ) -- ‘TySyntax⌜ Γ ⌝’
    ...
    (‘TermSyntax’ : ∀ {Γ} → TySyntax (Γ ▻ □𝒰))
    (⌜_⌝ : ∀ {Γ} → TySyntax Γ → TmSyntax {Γ} (𝟙 ‘→’ □𝒰))
    (‘quote’ : ∀ {Γ} → TmSyntax {Γ} (‘Σ’ □𝒰 ‘TermSyntax’ ‘→’ □ (‘Σ’ □𝒰 ‘TermSyntax’))) -- quotes the quoted type, and then the quoted term.  We should have `(‘quote’ ‘⨾’ ‘proj₁’) ≈ (proj₁ ⨾ quot)` (if that were a thing that typechecked)
    ...
    S : ∀ {Γ} → TySyntax Γ
    S = ‘Σ’ □𝒰 ‘TermSyntax’ -- ‘Σ’ □𝒰 ‘TermSyntax’ ‘→’ □𝒰
    quote-S : ∀ {Γ} → S {Γ} ~> □ S
    quote-S = ‘quote’
    ϕ : ∀ {Γ} → (S {Γ} ‘×’ □ S) ~> □𝒰
    ϕ = ({!!} ‘××’ ‘id’) ⨾ apply
