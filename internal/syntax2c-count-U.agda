{-# OPTIONS --sized-types --termination-depth=2 #-}
...
data Ty : (Γ : Con) → ℕ → Set
data Var : ∀ {i} (Γ : Con) → Ty Γ i → Set
data Tm : {i : ℕ} → (Γ : Con) → Ty Γ i → Set
...
postulate
  ...
  _≟t_ : ∀ {i Γ T} → SemiDecidable {A = Tm {i} Γ T} _≡_
  _≟T_ : ∀ {i Γ} → SemiDecidable {A = Ty Γ i} _≡_
  _≟v_ : ∀ {i Γ T} → SemiDecidable {A = Var {i} Γ T} _≡_

_▻'_ : ∀ {i} (Γ : Con) → Ty Γ i → Con
ε' : ∀ Γ → Γ ++

++_ : ∀ {Γ : Con} → Γ ++ → Con
_cw_ : ∀ {i} {Γ : Con} (A : Ty _ i) → Γ ++ → (Γ ▻' A) ++

_▻_ : ∀ {i Γ} (Δ : Γ ++) → Ty (++ Δ) i → Γ ++

Wₙ : ∀ {i j Γ} (Δ : Γ ++) (A : Ty Γ i) → Ty (++ Δ) j → Ty (++ A cw Δ) j
wₙ : ∀ {i j Γ} (Δ : Γ ++) (B : Ty Γ i) {A : Ty _ j} → Tm (++ Δ) A → Tm (++ B cw Δ) (Wₙ Δ B A)
vₙ : ∀ {i j Γ} (Δ : Γ ++) (B : Ty Γ i) {A : Ty _ j} → Var (++ Δ) A → Var (++ (B cw Δ)) (Wₙ Δ B A)
...
data Ty where
  U : ∀ {Γ} → Ty Γ 1
  El : ∀ {Γ} → Tm Γ U → Ty Γ 0 -- this constructor causes problems for termination of weakening
  ‘Π’ : ∀ {i j Γ} (A : Ty Γ i) → Ty (Γ , A) j → Ty Γ (i ⊔ℕ j)
--  ‘Σ’ : ∀ A → Ty (Γ , A) → Ty Γ

data Var where
  vz : ∀ {i Γ A}                 → Var {i} (Γ , A) (Wₙ (ε Γ) A A)
  vs : ∀ {i j Γ} {A : Ty _ i} {B : Ty _ j} → Var Γ A → Var {i} (Γ , B) (Wₙ (ε Γ) B A)
  v⊥ : ∀ {i} {Γ A}               → Var {i} Γ A

data Tm where
  var : ∀ {i Γ A} → Var {i} Γ A → Tm Γ A
...
{-∅ ≟c ∅ = just refl
∅ ≟c _ = nothing
(x , x₁) ≟c (y , x₂) = lift₂-≟ _,_ (_ ≟c _) (make-subst-SemiDec Ty _≟T_ x₁ x₂ (_ ≟c _))
(x , x₁) ≟c _ = nothing
...
cast-Var : ∀ {i} {Γ} {T T' : Ty Γ i} → Var Γ T → Var Γ T'
cast-Var {i} {Γ} {T} {T'} v with T ≟T T'
...
-- wₙU : ∀ {Γ} (Δ : Γ ++) (B : Ty Γ) → Tm (++ Δ) U → Tm (++ B cw Δ) U

Wₙ Δ A U = U
-- Wₙ Δ A (El x) = El (wₙ Δ A x)
Wₙ Δ A (‘Π’ {i} {j} {.(++ Δ)} ty ty') = {!‘Π’ (Wₙ Δ A ty) (Wₙ (Δ , ty) A ty')!}
Wₙ {Γ} Δ A (El x) = El {!(wₙU {Γ} Δ A x)!}
