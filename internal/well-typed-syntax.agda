...
mutual
  ...
  data Term : ∀ {Γ} → Typ Γ → Set where
    ...
    --- ewwwww, there must be a better way to do this than asking agda to normalize all the definitions and pasting them in here
    ‘context-pick-if’-refl-inv : {A : Typ ε} {T dummy : Typ (ε ▻ A)} {qqs : Term A} →
    {- normal form of ∀ {A T dummy qqs} →
      □ (‘□’ ‘’
          ((⌜ T ⌝T ‘‘’’ ⌜ qqs ⌝t)
             ‘‘→'’’
          ...
          (substTyp1-substTyp-weakenTyp
           (substTyp-weakenTyp
            (‘λ∙’
             (weakenTyp-substTyp-tProd
              (w
               (substTyp-weakenTyp (‘context-pick-if'’ ‘’ₐ ⌜ dummy ⌝T) ‘’ₐ
                ⌜ ε ▻ A ⌝c)))
             ‘’ₐ ⌜ ε ▻ A ⌝c)
            ‘’ₐ ⌜ T ⌝T)))
         ‘’ₐ ⌜ qqs ⌝t)))))
    ‘context-pick-if’-refl : {A : Typ ε} {T dummy : Typ (ε ▻ A)} {qqs : Term A} →
  {- normal form of ∀ {A T dummy qqs} →
      □ (‘□’ ‘’
          ((S₁₀WW (substTyp-tProd (‘context-pick-if'’ {ε} {ε ▻ A} ‘'’ₐ ⌜ dummy ⌝T ‘’ₐ ⌜ ε ▻ A ⌝c) ‘’ₐ ⌜ T ⌝T)
              ‘‘’’ ⌜ qqs ⌝t)
